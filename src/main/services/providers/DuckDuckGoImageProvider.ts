...
export class DuckDuckGoImageProvider {
  ...
  async searchImages(query: string, safeSearch: boolean = true): Promise<ImageResult[]> {
    try {
  ...
  private async searchImagesAlternative(query: string, safeSearch: boolean): Promise<ImageResult[]> {
    try {
      ...
      if (data && data.results && Array.isArray(data.results)) {
        const images: ImageResult[] = data.results
          ...
          .map((item: any, index: number) => ({
            url: item.image || `https://source.unsplash.com/400x300/?${encodeURIComponent(query)}&${index}`,
            thumbnail: item.thumbnail || `https://source.unsplash.com/200x150/?${encodeURIComponent(query)}&${index}`,
