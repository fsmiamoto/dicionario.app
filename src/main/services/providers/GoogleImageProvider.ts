...
export class GoogleImageProvider {
  ...
  async searchImages(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Google Custom Search API request failed: ${response.status} ${response.statusText} - ${errorText}`);
      }
  ...
  async validateCredentials(apiKey: string, searchEngineId: string): Promise<boolean> {
    try {
      ...
      console.error('Google Custom Search credentials validation failed:', error);
      return false;
    }
  }

  async getQuotaInfo(apiKey: string): Promise<{ used: number; limit: number } | null> {
