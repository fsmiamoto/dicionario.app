...
export class GoogleTTSProvider {
  ...
  private getCacheFilePath(text: string, language: string, voice?: string): string {
  ...
  async generateAudio(text: string, language: string = 'en-US', apiKey?: string, voice?: string): Promise<string> {
    try {
      // Check cache first
      ...
      if (!response.ok) {
        throw new Error(`Google TTS API request failed: ${response.status} ${response.statusText}`);
      }
  ...
  async getAvailableVoices(language: string, apiKey?: string): Promise<string[]> {
    try {
      if (!apiKey) {
        return ['default'];
      }
      ...
      const data = await response.json();
      return data.voices?.map((voice: any) => voice.name || 'default') || ['default'];
    } catch (error) {
