...
const SearchBar = forwardRef<SearchBarRef, SearchBarProps>(({ onSearch, isLoading }, ref) => {
  ...
  useEffect(() => {
    if (query.trim()) {
      const filtered = searchHistory
        .filter(item => item.word.toLowerCase().includes(query.toLowerCase()))
        .slice(0, 8);
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2 text-dark-400 text-sm">
                  ...
                  <span>•</span>
                  <span>{new Date(item.lastSearched).toLocaleDateString()}</span>
                </div>
              </div>
            </button>
          ))}
