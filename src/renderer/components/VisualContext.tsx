...
const VisualContext: React.FC<VisualContextProps> = ({ images, word, isLoading }) => {
      ...
      {isLoading ? (
        <div className="grid grid-cols-3 grid-rows-2 gap-3 aspect-[3/2]">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="bg-surface-300 rounded-lg animate-pulse aspect-square"></div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-3 grid-rows-2 gap-3 aspect-[3/2]">
